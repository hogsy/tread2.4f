...
CFileSearch::~CFileSearch() { EndFileSearch(); }

bool CFileSearch::BeginFileSearch(CString sDir, CString sFileExt,
                                  bool bRecursive) {
  m_sDir = sDir;
  m_sFileExt = sFileExt;
  ...
  if (!(pbRec.dirInfo.ioFlAttrib & kioFlAttribDirMask)) return false;
...
void CFileSearch::EndFileSearch() {
  KillRecursiveSearch();
#ifdef _WIN32
  if (m_nHandle > -1) _findclose(m_nHandle);
#endif
}
...
#ifdef _WIN32
  if (m_pSubSearch->BeginFileSearch(m_sDir + "\\" + m_fileinfo.name, m_sFileExt,
                                    true))
    return true;
#elif macintosh
  if (m_pSubSearch->BeginFileSearch(m_sDir + ":" + m_sCurFileName, m_sFileExt,
                                    true))
    return true;
#endif
...
bool CFileSearch::KillRecursiveSearch() {
  if (m_pSubSearch != NULL) delete m_pSubSearch;
  m_pSubSearch = NULL;
  return true;
}
...
bool CFileSearch::GetNextFile(CString& sFilename) {
  ...
  if (m_bFirstFind) {
    ...
    if (sFilename == ".." || sFilename == ".") return GetNextFile(sFilename);
  ...
  for (;;) {
    ...
    if (MatchesFileExt(szFilename, m_sFileExt)) break;
  }
...
bool ReadShort(CFile& hFile, int& nInt) {
  ...
  if (hFile.Read(&nNum, 2) != 2) return false;
...
bool WriteString(CFile& hFile, const char* szString) {
  ...
  if (nLen > 0) return WriteNString(hFile, szString, nLen);
  return true;
}
...
bool ReadLongString(CFile& hFile, CString& sString) {
  ...
  ReadInt(hFile, nLen);
  if (nLen > 0) return ReadNString(hFile, sString, nLen);
  sString = "";
  return true;
}
...
bool ReadVec3t(CFile& hFile, vec3_t vVec) {
  return ReadFloat(hFile, vVec[0]) && ReadFloat(hFile, vVec[1]) &&
         ReadFloat(hFile, vVec[2]);
}

bool WriteVec3t(CFile& hFile, vec3_t vVec) {
  return WriteFloat(hFile, vVec[0]) && WriteFloat(hFile, vVec[1]) &&
         WriteFloat(hFile, vVec[2]);
}
...
unsigned char* BloadFile(CString sFilename, int& nNumBytes) {
  ...
  if (!hFile.Open(sFilename,
                  CFile::modeRead | CFile::shareDenyWrite | CFile::typeBinary))
    return NULL;

  nNumBytes = static_cast<int>(hFile.GetLength());
  if (nNumBytes == 0) {
    hFile.Close();
    return NULL;
  }
...
CString FSGetAppPath() {
  CString regpath = GetRegString(S_TREAD_PATH);
  if (!regpath.IsEmpty()) {
    return regpath;
  }
